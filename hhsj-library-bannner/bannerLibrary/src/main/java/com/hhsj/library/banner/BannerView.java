package com.hhsj.library.banner;
...
import com.hhsj.library.R;
import com.hhsj.library.banner.config.BannerConfig;
import com.hhsj.library.banner.interfaces.IBannerUrl;
import com.hhsj.library.banner.transformer.DepthPageTransformer;
import com.hhsj.library.banner.transformer.RotateDownPageTransformer;
import com.hhsj.library.banner.transformer.ZoomOutPageTransformer;
import com.hhsj.library.utils.LOG;
...
//        mIndicatorHeight =typedArray.getDimensionPixelSize(R.styleable.Banner_indicator_height, BannerConfig.INDICATOR_SIZE);
    ...
    public void setImages(ArrayList<IBannerUrl> imagesUrl) {
        setImageList(imagesUrl, null);
    }

    public void setImages(IBannerUrl[] imagesUrl) {
        setImageArray(imagesUrl, null);
    }

    public void setImages(IBannerUrl[] imagesUrl, OnLoadImageListener imageListener) {
    ...
    private void setImageArray(IBannerUrl[] imagesUrl, OnLoadImageListener imageListener) {
        ...
        for (int i = 0; i <= count + 1; i++) {
            ...
            imageViews.add(iv);
            LOG.i(url.getUrl() + "=====((IBannerUrl) url).getUrl()=======");
...
//                    Glide.with(context).load(url).into(iv);
                    Glide.with(context).load(url.getUrl()).centerCrop().into(iv);
                } else {
                    Glide.with(context).load(url.getUrl()).centerCrop().into(iv);

//                    ImageLoader.getInstance().displayImage(((IBannerUrl) url).getUrl(), iv);
...
//                    Glide.with(context).load(url).placeholder(defaultImage).into(iv);
    ...
    private void setImageList(ArrayList<IBannerUrl> imagesUrl, OnLoadImageListener imageListener) {
...
//            iv.setTag(iv);
            imageViews.add(iv);
            LOG.i(url.getUrl() + "=====((PHOTO) url).getUrl()=======");
            if (imageListener != null) {
                imageListener.OnLoadImage(iv, url);
            } else {
                if (defaultImage != -1) {
...
//                    Glide.with(context).load(url).into(iv);
//                    ImageLoader.getInstance().displayImage(((IBannerUrl) url).getUrl(), iv);
                    Glide.with(context).load(((IBannerUrl) url).getUrl()).centerCrop().into(iv);
                } else {
                    Glide.with(context).load(((IBannerUrl) url).getUrl()).centerCrop().into(iv);
//                    ImageLoader.getInstance().displayImage(((IBannerUrl) url).getUrl(), iv);
//                    Glide.with(context).load(url).centerCrop().crossFade().placeholder(defaultImage).into(iv);
...
//            iv.setTag(iv);
            imageViews.add(iv);
            if (imageListener != null) {
                imageListener.OnLoadImage(iv, (IBannerUrl) url);
            } else {
                if (defaultImage != -1) {
//                    Glide.with(context).load(url).centerCrop().crossFade().into(iv);
//                    Glide.with(context).load(url).into(iv);
...
//                    Glide.with(context).load(url).placeholder(defaultImage).into(iv);
    ...
    public interface OnLoadImageListener {
        void OnLoadImage(ImageView view, IBannerUrl url);
    }
}
